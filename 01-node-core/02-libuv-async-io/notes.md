...
-> so, let's calculate, from starting after 5 minutes person-2's noodles will be ready, then again 5 minutes later both 5th person's noodles (5 minutes after person-2's noodles) & 3rd person's pizza (10 minutes after person-2's noodles) will be ready together, and then 5 minutes later 4th person's fries (15 minutes after person-2's noodles or 5 minutes after person-5's noodles / 5 minutes after person-3's pizza) will be ready.
...
6. **Garbage Collection**
   ...
   - GC clears multiplyFn's execution context (x, y, result)
   - Global vars (a, b, c) persist until program ends
...
2. **Continue Execution**
   ...
   - Output: "Next line..." runs immediately
...
5. **Memory Management**
   ...
   - GC cleans up res object after callback finishes
   - https module stays in memory for reuse
...
So, Node.js comes in play...  
JS accesses ☝️ above stuffs via: **"libuv"** (the SUPERHERO), which is given by Node.js.
...
```
JS code (e.g. API calls, readFiles, timeOut etc...)
            │
            ▼
            V8 Engine sends to
            ▼
            libuv
            │
            accesses OS features
            │
            gets response
            │
            sends response back to V8
            ▼
